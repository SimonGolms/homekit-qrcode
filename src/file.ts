...
const createImage = async (
  ...
  const svgBase64 = `data:image/svg+xml;base64,${Buffer.from(svg, 'binary').toString('base64')}`;

  return new Promise((resolve, reject) => {
    image.onload = () => {
      context.drawImage(image, 0, 0, 400, 540, 0, 0, width, height);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      resolve(canvas.toBuffer(mimetype as any));
    };
...
const createSvg = (pairingCode: string, qrCodeSvg: string) => {
...
<svg viewBox="0 0 400 540" xmlns="http://www.w3.org/2000/svg">
  ...
  <defs>
    ...
    <symbol id="qrCode" ${qrCodeSvg.slice(5, -7)}</symbol>
  </defs>
...
export const createFile = async (pairingCode: string, qrCodeSvg: string, output: OutputFormat): Promise<Buffer> => {
  const svg = createSvg(pairingCode, qrCodeSvg);

  if (output === 'svg') {
    return Promise.resolve(Buffer.from(svg, 'binary'));
  }

  return createImage(svg, `image/${output}`);
};
